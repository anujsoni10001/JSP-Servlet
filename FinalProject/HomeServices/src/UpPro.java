...
public class UpPro extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws NullPointerException,ServletException, IOException {
	    ...
	    case "tyservice":qr="update addser set tyservice=? where id=?";
		    	  ...
		    	  if(i!=0)
		    	  {
		    	  RequestDispatcher rd=request.getRequestDispatcher("UpPro2.html");
	    ...
	    case "Location":qr="update addser set Location=? where id=?";
    	            ...
    	            i=ps.executeUpdate();
  	                if(i!=0)
  	                {
  	                RequestDispatcher rd=request.getRequestDispatcher("UpPro2.html");  	 			    rd.include(request, response);
  	                pw.println("Updation By ID Succesful");
  	                }
  	                else
  	                {
