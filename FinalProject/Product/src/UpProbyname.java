...
public class UpProbyname extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/eshop","root","root");
		 switch(cat)
		    {
		    case "id":qr="update Product set id=? where Name=?";
			    	  ...
			    	  ps1.close();
		    	      break;
		    case "Price":qr="update Product set Price=? where Name=?";
				         ps1=con.prepareStatement(qr);
	                     ps1.setString(1,catuval);
	                     ps1.setString(2,name);
	    	             i=ps1.executeUpdate();
	  	                 ...
	  	                 ps1.close();
		                 break;
		    case "Category":qr="update Product set Category=? where Name=?";
	                        ps1=con.prepareStatement(qr);
	                        ps1.setString(1,catuval);
	                        ps1.setString(2,name);
	                        i=ps1.executeUpdate();
	                        ...
	                        ps1.close();
	                        break;
		    case "Company":qr="update Product set Company=? where Name=?";
	                       ps1=con.prepareStatement(qr);
	                       ps1.setString(1,catuval);
	                       ps1.setString(2,name);
	                       i=ps1.executeUpdate();
